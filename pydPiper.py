		...
		while not exitapp[0]:
			...
			with self.musicdata_lock:
				...
				if self.musicdata[u'elapsed'] != self.musicdata_prev[u'elapsed']:
					...
					#timepos_tot = time.strftime("%-M:%S", time.gmtime(self.musicdata[u'total_time']))
					...
					#self.musicdata[u'total_simple'] = timepos_tot.decode()
			...
			# Update display data every 4 seconds
			time.sleep(1)
...
#    		sys.stdout = codecs.getwriter(u'utf-8')(sys.stdout, u'strict')
	...
	logging.info(pydPiper_config.LOGFILE)
